...
export default function CitationList({ setCitationData, handleClose }) {
  ...
  useEffect(() => {
    if (citationStatus === 'success') {
      const newCitationList = citationData?.data?.filter((data) => data.invoice.status === 'unpaid');
      setCitationList(
        newCitationList.map((data) => ({
          action: (
            <Box sx={{ width: 100 }}>
          ...
          violations: (
            <>
              {violationsList?.map((violations, _index) => {
